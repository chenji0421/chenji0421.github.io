...
## 2. 前端（未来的 React/Vite）→ Vercel / Netlify（未来）
...
## 3. 后端（FastAPI）→ Render / Railway / 云服务器（未来）

```bash
cd backend            # 未来重新建立 backend/ 时
...
**Q：想换网站的主题色？**
A：改 `css/style.css` 顶部的 `--accent`（青绿）和 `--accent-2`（蓝），全站颜色统一变化。
