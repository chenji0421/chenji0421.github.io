...
| 🗓️ **计划** | 浏览器 `localStorage` | 计划页是「年 / 月 / 日」三视图，数据存在你自己的浏览器里，**不会上传服务器**。 |
...
| 🗓️ **计划** | 个人计划本：年表（12 个月份卡片）+ 月表（真实日历）+ 日计划（目标 / 上午 / 下午 / 晚上 / 复盘 / 状态）。数据存 localStorage，支持导出 / 导入 / 清空 |
...
| 数据 | `js/content.js`（空框架）+ `localStorage`（计划 / 主题 / 侧边栏状态） |
...
- 计划页的年 / 月 / 日计划数据（键名 `chenji_planner_data`）
- 主题偏好（键名 `hub_theme`）
...
**使用计划系统：**
1. 打开「计划」页，在「年表」点某个月进入「月表」
2. 点某一天进入「日计划」，填写目标 / 上午 / 下午 / 晚上 / 复盘 / 状态
3. 点「保存」，数据存在你自己的浏览器里
4. 可以用「导出 JSON / 导入 JSON」备份到本地
